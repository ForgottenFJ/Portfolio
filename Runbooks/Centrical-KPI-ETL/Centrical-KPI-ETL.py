...
if __name__ == "__main__":
    ...
    # --- Generate universal FirstDayofWeek, FirstDayofMonth, FirstDayofYear in final_df ---
    logging.info("Generating universal FirstDayofWeek, FirstDayofMonth, FirstDayofYear columns.")

    # Initialize date columns with NA/NaN
    final_df['FirstDayofWeek'] = pd.NaT  
    final_df['FirstDayofMonth'] = pd.NaT
    final_df['FirstDayofYear'] = pd.NA  

    if 'CreatedDate' in final_df.columns:
        # Use the existing CreatedDate column which should be datetime.date objects
        # Convert to datetime objects for period and year extraction, then back to date if needed
        temp_created_date_dt = pd.to_datetime(final_df['CreatedDate'])  

        # Identify weekly KPI columns
        weekly_kpi_cols = []
        # Identify monthly KPI columns
        monthly_kpi_cols = []
        # Identify yearly KPI columns
        yearly_kpi_cols = []
        ...
        for col in final_df.columns:
            for prefix_val in [config[1] for config in litigation_configs]:
                if col.startswith(prefix_val):
                    kpi_suffix = col[len(prefix_val):]
                    if 'WEEKLY' in kpi_suffix and kpi_suffix in [k.upper() for k in base_kpi_cols]:
                        weekly_kpi_cols.append(col)
                    elif 'MONTHLY' in kpi_suffix and kpi_suffix in [k.upper() for k in base_kpi_cols]:
                        monthly_kpi_cols.append(col)
                    elif 'YEARLY' in kpi_suffix and kpi_suffix in [k.upper() for k in base_kpi_cols]:
                        yearly_kpi_cols.append(col)
                    break 

        # Create separate masks for each date column based on corresponding KPIs
        # Now we need to check for values > 0 since nulls have been filled with 0
        has_weekly_kpi_mask = (final_df[weekly_kpi_cols] > 0).any(axis=1) if weekly_kpi_cols else pd.Series(False, index=final_df.index)
        has_monthly_kpi_mask = (final_df[monthly_kpi_cols] > 0).any(axis=1) if monthly_kpi_cols else pd.Series(False, index=final_df.index)
        has_yearly_kpi_mask = (final_df[yearly_kpi_cols] > 0).any(axis=1) if yearly_kpi_cols else pd.Series(False, index=final_df.index)

        # For monthly KPIs, we need to ensure only one FirstDayofMonth per user per month
        # Find the latest date in each month for each user that has monthly KPI activity
        if monthly_kpi_cols:
            # Get rows that have monthly KPI activity
            monthly_activity_df = final_df[has_monthly_kpi_mask].copy()
            
            if not monthly_activity_df.empty:
                # Add month-year grouping column
                monthly_activity_df['MonthYear'] = pd.to_datetime(monthly_activity_df['CreatedDate']).dt.to_period('M')
                
                # Find the latest date for each user in each month that has monthly activity
                latest_monthly_dates = monthly_activity_df.groupby(['Email', 'MonthYear'])['CreatedDate'].max().reset_index()
                latest_monthly_dates['FirstDayofMonth_Value'] = pd.to_datetime(latest_monthly_dates['CreatedDate']).dt.to_period('M').apply(lambda r: r.start_time.date())
                
                # Create a refined mask for monthly KPIs - only True for the latest date in each month per user
                refined_monthly_mask = pd.Series(False, index=final_df.index)
                for _, row in latest_monthly_dates.iterrows():
                    mask = (final_df['Email'] == row['Email']) & (final_df['CreatedDate'] == row['CreatedDate'])
                    refined_monthly_mask |= mask
                
                # Use the refined mask instead of the original monthly mask
                has_monthly_kpi_mask = refined_monthly_mask
                
                # Clean up temporary dataframe
                del monthly_activity_df, latest_monthly_dates
                gc.collect()

        # Similarly for weekly KPIs, ensure only one FirstDayofWeek per user per week
        if weekly_kpi_cols:
            # Get rows that have weekly KPI activity
            weekly_activity_df = final_df[has_weekly_kpi_mask].copy()
            
            if not weekly_activity_df.empty:
                # Add week grouping column
                weekly_activity_df['WeekYear'] = pd.to_datetime(weekly_activity_df['CreatedDate']).dt.to_period('W-SAT')
                
                # Find the latest date for each user in each week that has weekly activity
                latest_weekly_dates = weekly_activity_df.groupby(['Email', 'WeekYear'])['CreatedDate'].max().reset_index()
                latest_weekly_dates['FirstDayofWeek_Value'] = pd.to_datetime(latest_weekly_dates['CreatedDate']).dt.to_period('W-SAT').apply(lambda r: r.start_time.date())
                
                # Create a refined mask for weekly KPIs - only True for the latest date in each week per user
                refined_weekly_mask = pd.Series(False, index=final_df.index)
                for _, row in latest_weekly_dates.iterrows():
                    mask = (final_df['Email'] == row['Email']) & (final_df['CreatedDate'] == row['CreatedDate'])
                    refined_weekly_mask |= mask
                
                # Use the refined mask instead of the original weekly mask
                has_weekly_kpi_mask = refined_weekly_mask
                
                # Clean up temporary dataframes
                del weekly_activity_df, latest_weekly_dates
                gc.collect()

        # For yearly KPIs, ensure only one FirstDayofYear per user per year
        if yearly_kpi_cols:
            # Get rows that have yearly KPI activity
            yearly_activity_df = final_df[has_yearly_kpi_mask].copy()
            
            if not yearly_activity_df.empty:
                # Add year grouping column
                yearly_activity_df['Year'] = pd.to_datetime(yearly_activity_df['CreatedDate']).dt.year
                
                # Find the latest date for each user in each year that has yearly activity
                latest_yearly_dates = yearly_activity_df.groupby(['Email', 'Year'])['CreatedDate'].max().reset_index()
                latest_yearly_dates['FirstDayofYear_Value'] = latest_yearly_dates['Year'].apply(lambda y: datetime(y, 1, 1).date())
                
                # Create a refined mask for yearly KPIs - only True for the latest date in each year per user
                refined_yearly_mask = pd.Series(False, index=final_df.index)
                for _, row in latest_yearly_dates.iterrows():
                    mask = (final_df['Email'] == row['Email']) & (final_df['CreatedDate'] == row['CreatedDate'])
                    refined_yearly_mask |= mask
                
                # Use the refined mask instead of the original yearly mask
                has_yearly_kpi_mask = refined_yearly_mask
                
                # Clean up temporary dataframes
                del yearly_activity_df, latest_yearly_dates
                gc.collect()

        # Conditionally populate date columns using the refined masks
        final_df.loc[has_weekly_kpi_mask, 'FirstDayofWeek'] = temp_created_date_dt[has_weekly_kpi_mask].dt.to_period('W-SAT').apply(lambda r: r.start_time.date())
        final_df.loc[has_monthly_kpi_mask, 'FirstDayofMonth'] = temp_created_date_dt[has_monthly_kpi_mask].dt.to_period('M').apply(lambda r: r.start_time.date())
        final_df.loc[has_yearly_kpi_mask, 'FirstDayofYear'] = temp_created_date_dt[has_yearly_kpi_mask].dt.year.apply(lambda y: datetime(y, 1, 1).date())

    else:
        logging.warning("CreatedDate not found in final_df, cannot generate date columns.")
